...
public class DefaultSelectProcessorImpl extends SQLProcessorImpl implements SQLSelectProcessor {
	...
	public List execute(SQLSelect select){
		...
		if (fields!=null){
			...
			for (SQLField sqlField : fields) {
				...
				//Esto solo deber�a pasar en primeras fases de desarrollo
				if (sqlColumn == null){
					//"El atributo ["+sqlField.getField()+"] de la clase ["+model.getClassName()+"] no est� mapeado."
	...
	//		String schema = modelManager.getSchemaName();
			...
			if (from.getTables().size() > 1){
				//" Hay m�s de un from. Se han de indicar qu� campos se quieren recuperar " + sql
...
//		}
	...
	private void checkOuputModel(SQLSelect select, SQLFrom from,
		...
		if (outputModel == null){
			...
			if (from.getTables().size() > 1){
				//" No se ha indicado un modelo de salida de la query, al tener m�s de una tabla no se puede asignar uno autom�ticamente: " + sql
	...
	private void processPagination(SQLSelect select, StringBuilder sql) {
		...
		if (pagination == null){
			//comprobamos la paginaci�n global
	 ...
	 * <p>Si retorna más de un objeto se lanza una <code>DAOException</code> con el c�digo 
	 * <code>ErrorMessageID.PE_00001</code>.</p>
	 * <p>Si se quiere obtener más de una instancia utilizar los m�todos <code>find</code>.
	...
	private List getDynamicWhereFromObject(Object object, ModelDefinition model, StringBuilder sql) {
		...
		Map<String,Relationship> relations =model.getRelations();
		//primero se han de procesar las relaciones con inner join y despu�s los campos "normales"
		List params = new ArrayList();
...
//				}
	...
	/**
	 * <p>A�ade a <code>sqlWhere</code> las condiciones de filtro indicadas en <code>object</code>.</p>
	 * @param object
	 * @param model
	 * @param sqlWhere
	 * @return sqlWhere
	...
	private void processInnerJoin(SQLSelect select, StringBuilder sql, List params){
		...
		.append( SQLConstants.EQUAL) //TODO esto deber�a poder especificarlo el usuario
		.append(targetModel.getFullTableName())
		.append(SQLConstants.DOT)
...
//				index++;
	...
	private void processHaving(SQLHaving having, StringBuilder sql, List params){
		...
		//El render de la condici�n incluye el AND, por eso poneos 1=1
